...
main <- function() {
    ...
    schemas <- fromJSON("source/lib/schemas.json")

    quarters <- get_quarters(start_date, end_date)

    for (quarter in quarters) {
        in_file <- file.path(INDIR, paste0(quarter, ".csv"))
        names <- names(schemas$fannie_mae)
        classes <- unlist(schemas$fannie_mae, use.names = FALSE)

        cat("Processing:", paste0(quarter, ".csv"), "\n")

        df <- load_file(in_file, names, classes)

        df_base <- prepare_base_data(df)

        acquisition_date <- create_acquisition_date(quarter)
        acquisition_file <- create_acquisition_file(df_base)
        performance_file <- create_performance_file(df_base)
        base_table_1 <- create_base_table_1(acquisition_file, acquisition_date)
        ...
        base_table_4 <- create_base_table_4(base_table_3, performance_file, acquisition_file)
        base_table_5 <- create_base_table_5(base_table_4)
        base_table_6 <- create_base_table_6(base_table_1, base_table_5, performance_file)
        final_output <- create_final_output(base_table_6)
...
get_quarters <- function(start_date, end_date) {
    if (missing(start_date) || missing(end_date)) {
        stop("Please provide `start_date` and `end_date`")
    }

    start_date <- as.Date(start_date)
    end_date <- as.Date(end_date)

    seq_dates <- seq(from = floor_date(start_date, "quarter"), to = ceiling_date(end_date, "quarter") - days(1), by = "quarter")
    quarters <- paste0(year(seq_dates), "Q", quarter(seq_dates))
    return(quarters)
}


load_file <- function(filename, col_names, col_classes) {
    df <- fread(filename, sep = "|", col.names = col_names, colClasses = col_classes)
    df$ORIG_RATE <- as.numeric(df$ORIG_RATE)
    df$CURR_RATE <- as.numeric(df$CURR_RATE)
    return(df)
}


prepare_base_data <- function(df) {
...
create_acquisition_date <- function(quarter) {
    acquisition_year <- substr(quarter, 1, 4)
    acquisition_qtr <- substr(quarter, 5, 7)
...
create_acquisition_file <- function(df) {
    ...
    acqFirstPeriod <- acquisitionFile %>%
        group_by(LOAN_ID) %>%
        summarize(first_period = max(ACT_PERIOD)) %>%
...
create_performance_file <- function(df) {
...
create_base_table_1 <- function(acquisition_file, acquisition_date) {
...
create_base_table_2 <- function(base_table_1, performance_file) {
    ### Create the second base table with the latest-available or aggregated data from the Performance fields
    last_act_dte_table <- performance_file %>%
        group_by(LOAN_ID) %>%
        summarize(LAST_ACTIVITY_DATE = max(period))

    last_upb_table <- performance_file %>%
        group_by(LOAN_ID) %>%
        filter(period == max(period)) %>%
        summarize(LAST_UPB = ifelse(!is.na(zb_upb), zb_upb, act_upb))

    last_rt_table <- performance_file %>%
        filter(!is.na(curr_rte)) %>%
        group_by(LOAN_ID) %>%
        summarize(
            LAST_RT_DATE = max(period)
        ) %>%
        left_join(performance_file, by = c("LOAN_ID" = "LOAN_ID", "LAST_RT_DATE" = "period")) %>%
        select(LOAN_ID, curr_rte) %>%
        rename(LAST_RT = curr_rte) %>%
        mutate(LAST_RT = round(LAST_RT, 3))

    zb_code_table <- performance_file %>%
        filter(z_zb_code != "") %>%
        group_by(LOAN_ID) %>%
        summarize(zb_code_dt = max(period)) %>%
        left_join(performance_file, by = c("LOAN_ID" = "LOAN_ID", "zb_code_dt" = "period")) %>%
        select(LOAN_ID, z_zb_code) %>%
        rename(zb_code = z_zb_code)

    max_table <- last_act_dte_table %>%
        left_join(performance_file, by = c("LOAN_ID" = "LOAN_ID", "LAST_ACTIVITY_DATE" = "period")) %>%
    ...
    servicer_table <- performance_file %>%
        filter(servicer != "") %>%
        group_by(LOAN_ID) %>%
        summarize(servicer_period = max(period)) %>%
        left_join(performance_file, by = c("LOAN_ID" = "LOAN_ID", "servicer_period" = "period")) %>%
        mutate(SERVICER = servicer) %>%
        select(LOAN_ID, SERVICER)

    non_int_upb_table <- performance_file %>%
        group_by(LOAN_ID) %>%
        slice(n() - 1) %>%
        select(LOAN_ID, NON_INT_UPB = non_int_upb) %>%
        mutate(NON_INT_UPB = if_else(is.na(NON_INT_UPB), 0, NON_INT_UPB))


    baseTable2 <- base_table_1 %>%
...
create_base_table_3 <- function(base_table_2, performance_file) {
    ### Create the third base table with the latest-available forclosure/disposition data
    fcc_table <- performance_file %>%
...
create_base_table_4 <- function(base_table_3, performance_file, acquisition_file) {
    ### Create the series of "first DQ occurence" tables and loan modification tables
    slimPerformanceFile <- performance_file %>%
    ...
    num_120_table <- f120_table %>%
        left_join(acquisition_file, by = "LOAN_ID") %>%
        mutate(
...
create_base_table_6 <- function(base_table_1, base_table_5, performance_file) {
    ### Compute MODIR_COST, MODFB_COST and other loan modification fields
    modir_table <- base_table_1 %>%
        left_join(performance_file, by = "LOAN_ID") %>%
        filter(mod_ind == "Y") %>%
        mutate(
...
create_final_output <- function(base_table_6) {
